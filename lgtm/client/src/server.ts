...
app.get('/orders', (req, res) => {
  ...
  // Simulate order data
  const orders = [
    { id: 1, status: 'pending', amount: 99.99, createdAt: new Date().toISOString() },
    { id: 2, status: 'completed', amount: 149.50, createdAt: new Date().toISOString() },
    { id: 3, status: 'cancelled', amount: 75.25, createdAt: new Date().toISOString() }
  ];
...
app.get('/test-direct-otlp', async (req, res) => {
  try {
        ...
        scopeLogs: [{
          ...
          logRecords: [{
            ...
            attributes: [
              ...
              { key: "endpoint", value: { stringValue: "/test-direct-otlp" } }
            ]
          }]
        }]
      }]
    ...
    const response = await fetch(process.env.OTLP_ENDPOINT || 'http://localhost:4318/v1/logs', {
      method: 'POST',
      headers: {
...
const server = app.listen(port, () => {
  ...
  console.log(`📡 OTLP logs sending to: ${process.env.OTLP_ENDPOINT || 'http://localhost:4318/v1/logs'}`);
});
